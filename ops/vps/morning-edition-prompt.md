...
- X capture is pre-run by the wrapper with the read-only wrapper. Never mutate X.
...
5. Inspect ops/raw/DATE_PLACEHOLDER-x-candidates.json if it exists. If it is missing, try: BIRD_READONLY=/home/sushil/.local/bin/bird-readonly node ops/scripts/capture-x-candidates.mjs DATE_PLACEHOLDER.
...
9. Add 1-2 X fragments only if high-signal candidates are available before cutoff. If not, leave signals.tweets as []; the wrapper will run ops/scripts/select-x-fragment.mjs once more after your edit and attach one qualifying fragment if the deterministic filter finds it.
